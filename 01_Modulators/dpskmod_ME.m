% This function performs DPSK modulation on a binary vector.
%
...
%
% Output:
%   y: Complex baseband signal as a row vector with length (L+1)*Mod.sps
%
% References:
...
function y = dpskmod_ME(Mod, data)
...
Phase_Constellation(Phase_Constellation>pi) = Phase_Constellation(Phase_Constellation>pi)-2*pi;

%% Symbol Mapping
v = symbolmap([0 ; bi2de(vec2mat(data,log2(M)),'left-msb')]+1);

%% Modulation
S = exp(1i*cumsum(Phase_Constellation(v+1))); % modulator output signal
