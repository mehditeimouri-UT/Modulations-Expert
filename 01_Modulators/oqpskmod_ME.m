% This function performs OQPSK modulation on a binary vector.
%
...
%
% Output:
%   y: Complex baseband signal as a row vector with length L*Mod.sps+Mod.half_symbol
%
% References:
...
function y = oqpskmod_ME(Mod, data)
...
Phase_Constellation = (phaserot+2*pi*(0:M-1)/M); % Phase of constellation points

%% Symbol Mapping
v = symbolmap(bi2de(vec2mat(data,log2(M)),'left-msb')+1);

%% Modulation
S = exp(1i*Phase_Constellation(v+1)); % modulator output signal
x = conv(upsample(S,sps),h); % upsampling and pulse shaping

% remove redundant samples and add an extra delay of T/2 seconds in the Q-channel 
x = [real(x(delay+1-half_symbol:end-delay-half_symbol)) zeros(1,half_symbol)]+...
    1i*[zeros(1,half_symbol) imag(x(delay+1-half_symbol:end-delay-half_symbol))];

%% Apply frequency and phase offset
y = complex(x.*exp(1i*2*pi*dfcTs/sps*(0:numel(x)-1)+1i*ini_phase)); % carrier modulation