% This function performs modulation for upper-sideband (USB) modulation on a complex baseband signal.
%
...
%% Remove nagative frequencies
X = fft(x); 
F = (0:length(x)-1)/length(x); 

X(F>0.5) = 0; 
X(F<0.5 & F>0) = 2*X(F<0.5 & F>0); 

x_usb = ifft(X);

%% Modulation and applying frequency and phase offset
n = (0:numel(x_usb)-1);
y = complex(x_usb.*exp(1i*2*pi*dfc_div_fs*n+1i*ini_phase));