function ErrorMsg = Script_Present_Channel_Output_ME

% This function presents channel output in ones of the
% following ways:
...
%% Initialization
global channel_output_ME iq_information_ME
global modulator_ME
...
[ErrorMsg,Subsets,~] = Select_from_List_ME({'Power Spectral Density','Signal Information','Save to WAV File','Pass Signal to AMR'},1,'Select Presentation Type for Channel Output',true);
if ~isempty(ErrorMsg)
    return;
end
...
switch Subsets{1}
    ...
    case 1 % Power Spectral Density
        ...
        ErrorMsg = Plot_PSD_ME(channel_output_ME.Content,channel_output_ME.fs);
        if ~isempty(ErrorMsg)
            return;
        end
    ...
    case 2 % Signal Information
        ...
        Display_Sturcture_Info_ME(channel_output_ME,'Channel Output');
    ...
    case 3 % Save to WAV File
        ...
        pause(0.01);
        x = channel_output_ME.Content;
        x = x/max(abs(x));
        audiowrite([PathName FileName],[real(x) ; imag(x)]',channel_output_ME.fs);
        GUI_MainEditBox_Update_ME(false,'WAV File is successfully written.');
    
    case 4 % Pass Signal to AMR
        
        if length(channel_output_ME.Content)<MinimumBurstLength
        ...
        % Pass Signal
        iq_information_ME = [];
        iq_information_ME.Content = channel_output_ME.Content;
        iq_information_ME.fs = channel_output_ME.fs;
