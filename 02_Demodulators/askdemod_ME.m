% This function performs ASK demodulation on a complex baseband signal.
%
...
%       equal to L*Mod.sps and less than (L+1)*Mod.sps
%
% Output:
...
function [data,s_hat] = askdemod_ME(Mod, y)
...
ModSymbols = (-M+1:2:M-1).*exp(1i*phaserot); % Constellation points
energy = mean(abs(ModSymbols).^2);
ModSymbols = ModSymbols/sqrt(energy);
...
%% Demodulation
s_hat = y(half_symbol+1:sps:end); 

L = floor(length(y)/sps);
s_hat = s_hat(1:L);
